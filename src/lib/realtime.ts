import type { AmeenArtifact, AmeenToolCall, AmeenToolResult, AmeenToolSpec } from "../vite-env";

export type AmeenConnectionState = "idle" | "connecting" | "connected" | "error";
export type AmeenMood = "idle" | "listening" | "thinking" | "speaking" | "working" | "error";
...
export type TranscriptEntry = {
  id: string;
  role: "user" | "ameen" | "system" | "tool";
  text: string;
  at: string;
};

export type RealtimeCallbacks = {
  onConnectionState: (state: AmeenConnectionState) => void;
  onMood: (mood: AmeenMood) => void;
  onMouthShape: (shape: MouthShape) => void;
  onTranscript: (entry: TranscriptEntry) => void;
  onArtifact: (artifact: AmeenArtifact) => void;
...
export class AmeenRealtimeClient {
  private pc: RTCPeerConnection | null = null;
  private dc: RTCDataChannel | null = null;
  private micStream: MediaStream | null = null;
  private callbacks: RealtimeCallbacks;
  private currentAssistantText = "";
  private toolSpecs: AmeenToolSpec[] = [];
  ...
  async connect(): Promise<void> {
    ...
    try {
      this.toolSpecs = await window.ameen.getToolSpecs();
      const token = await window.ameen.createRealtimeToken();
      ...
      dc.addEventListener("open", () => {
        ...
        this.callbacks.onStatus("Ameen is live. Start talking naturally.");
      });
  ...
  sendText(text: string): void {
    if (!this.dc || this.dc.readyState !== "open") {
      this.callbacks.onStatus("Connect Ameen before sending a text prompt.");
      return;
    }
  ...
  private async handleServerEvent(raw: string): Promise<void> {
    ...
    if (event.type === "response.done") {
      ...
      if (spoken) this.callbacks.onTranscript(newEntry("ameen", spoken));
      this.currentAssistantText = "";
    ...
    for (const item of items) {
      ...
      if (name === "image_generate") {
        this.callbacks.onArtifact({
          ...
          kind: "imageLoading",
          content: typeof parsedArgs.prompt === "string" ? parsedArgs.prompt : "Ameen is generating an image.",
        });
      }
      if (name === "thumbnail_generate" || name === "thumbnail_edit") {
        const loadingResult = await window.ameen.executeTool({
          name: "thumbnail_loading_prepare",
          arguments: {
            ...parsedArgs,
            mode: name === "thumbnail_edit" ? "edit" : "generate",
          },
        } satisfies AmeenToolCall);
      ...
      const result = await window.ameen.executeTool({ name, arguments: parsedArgs } satisfies AmeenToolCall);
  ...
  private async returnToolOutput(callId: string, result: AmeenToolResult): Promise<void> {
    this.sendEvent({
      type: "conversation.item.create",
      item: {
...
function sanitizeToolResult(result: AmeenToolResult): AmeenToolResult {
